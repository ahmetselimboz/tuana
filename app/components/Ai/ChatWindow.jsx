...
const ChatWindow = ({
  ...
  return (
    <div className={`flex-1 relative overflow-y-scroll ${isFullScreen ? "h-full":"h-4/6"}  overflow-x-hidden select-text`} ref={messagesContainerRef}>
      {/* Mesajlar */}
      <div className="flex-1 overflow-y-hidden p-4 overflow-x-hidden select-text">
        {messages.map((message, index) => (
          <div
            key={index}
            className={`mb-4 flex lg:text-base text-sm select-text ${
              message.sender === 'user' ? 'justify-end' : 'justify-start'
            }`}
          >
            <div
              className={`max-w-xs px-4 py-1 rounded-lg shadow-md select-text ${
                message.sender === 'user'
      ...
      {/* Kullanıcı hakları */}
      {chatRights.limitExist && showRightsField && (
        chatRights.limit !== 0 ? (
          <div className={`flex items-center fixed  bottom-11 left-1/3 bg-main px-4 py-1 rounded-tl-lg rounded-tr-lg shadow-md`}>
          ...
          <div className={`flex items-center fixed  bottom-3 left-1/3 text-red-500 bg-main px-4 py-1 shadow-md rounded-tl-lg rounded-tr-lg`}>
            <RiErrorWarningLine />
            <small className="ml-1">
              You have <b>{chatRights.limit}</b> chat rights
            </small>
      ...
      {chatRights.limit !== 0 ? (
        <div className={`fixed bottom-0 w-full pb-2 px-4 bg-main flex flex-col items-center `}>
