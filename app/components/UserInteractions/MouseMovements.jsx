...
const MouseMovements = () => {
  ...
  useEffect(() => {
    if (loading) {
      ...
      console.log("🚀 ~ useEffect ~ heatmapData:", heatmapData)
  ...
  useEffect(() => {
    ...
    heatmapData?.forEach((page, index) => {
      const containerId = `heatmap-${index}`;
  ...
  return (
    <div className='w-full flex flex-col items-start mb-10'>
      ...
      <div className="w-full flex lg:flex-row flex-col items-center rounded-md shadow-xl border border-stone-900/20 bg-main py-4 px-2">
        {heatmapData?.map((page, index) => (
          ...
          >
            {/* URL Başlığı */}
            <h3 className="text-lg font-semibold mb-4 text-center">{page.url}</h3>
